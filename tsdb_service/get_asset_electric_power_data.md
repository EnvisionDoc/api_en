...
|-------|----------------|---------------------------|
| **items** | `List<Object>` | List of asset data. The data returned for a single point of a single device is sorted by time in ascending order. Parameters are stored in the Object struct. See [items](/docs/api/en/2.0.9/tsdb_service/get_asset_electric_power_data.html#id3).  |
...
```json
{
       ...
       "sum(opentsdb_pi_point_xxx)": "4.246",  	//measurement point identifier and the data, here is the daily aggregation of the sum of power pi
       "localtime": "6/11/2019 6:35:00 PM"	//local time mark of data		
